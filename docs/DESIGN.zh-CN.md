# 本地调试平台设计文档 —— `local-debug (ldbg)`

> 版本：v2.0（2026-08-04）　对应实现版本：`ldbg v0.3.4`　状态：**已实现并验证**
>
> v1.1（2026-07-05）只覆盖「`ldbg` × `log-analysis` 日志整合」这一个工作流。
> v2.0 把此后交付的三个子系统补入本文档：**远程接入**（v0.2.0–v0.2.1）、
> **离线镜像交付**（v0.3.0–v0.3.2）、**集群侧导入与安装**（v0.3.3–v0.3.4）。
>
> 相关仓库：
> - [`github.com/hzeng10/local-debug`](https://github.com/hzeng10/local-debug) —— `ldbg`，基于 Telepresence 全量拦截的本地调试命令行工具
> - [`github.com/hzeng10/log-analysis`](https://github.com/hzeng10/log-analysis) —— 集群日志分析系统（VictoriaLogs + Vector + Grafana + `logq` 命令行工具，v0.1.1）

## 文档结构导航

| 你想了解 | 请读 |
| --- | --- |
| **需求是什么、怎么算做到了** | [`SPEC.zh-CN.md`](SPEC.zh-CN.md)（需求规格说明书，逐条编号 + 验收标准） |
| 调试闭环与日志整合的设计与论证 | 本文档 §1–§13（v1.1 原有内容，仍然有效） |
| 全局模块划分（当前代码结构） | 本文档 §14 |
| 离线镜像打包（`ldbg bundle`）的设计 | 本文档 §15 |
...
# 第二部分：远程接入、离线交付与集群导入（v2.0 新增）

> 以下章节描述 v1.1 之后交付的三个子系统。它们与第一部分（调试闭环 + 日志）共用同一套
> 命令行框架、同一套统一 JSON 外壳与同一套凭证处理原则。

## 14. 全局模块划分

代码分为两层：`cmd/` 只负责解析参数与渲染输出，真正的逻辑都在 `internal/` 中，
以便对纯逻辑做单元测试而不触发任何外部副作用。

| 包 | 职责 | 关键说明 |
| --- | --- | --- |
| `cmd/` | 每个子命令一个文件；解析参数、按需调用 `internal/*`、通过统一打印器输出 | 命令体保持"薄"：不写业务判断，只做编排与错误提示的选择 |
| `internal/output` | 统一 JSON 外壳与人类可读输出的唯一出口；秘密值打码 | 所有命令都必须经它输出，外壳形状才不会各自漂移 |
| `internal/k8s` | 与集群交互：工作负载解析、环境变量解析、Istio ambient 标签、端口转发、集群内探测、**节点事实**（v0.3.3 新增） | 基于 Kubernetes 官方客户端库；有权限失败时尽量降级而不是整体失败 |
| `internal/tp` | Telepresence 命令行的封装：连接、拦截、退出、Helm 安装与升级 | 只做进程调用与输出解析，不做策略判断 |
| `internal/springconfig` | 集群环境变量导出为 env-file，供 Spring Boot 的宽松绑定覆盖配置 | 秘密值完整写入受限权限文件，终端输出打码 |
| `internal/locallog` | 拦截期间本机日志的落盘与解析 | 见 §6.5 |
| `internal/logquery` | 内嵌自 `log-analysis` 的查询层（零依赖） | 见 §6.2 |
| `internal/mesh` | Istio ambient 的评估逻辑（是否需要豁免） | 纯函数，可单测 |
| `internal/offline` | **离线交付的全部逻辑**：镜像打包、代理解析、运行时命令、节点传输、仓库推送 | v0.3.x 的主要新增；纯逻辑与副作用严格分离 |

`internal/offline` 内部再分为六个关注点，每个都有对应的单元测试文件：

| 文件 | 关注点 |
| --- | --- |
| `offline.go` | 镜像名与平台的解析、输出文件命名、本地 Docker 能力探测、**失败分类与可操作提示** |
| `registry.go` | 直接与镜像仓库通信的拉取（不依赖 Docker） |
| `push.go` | 直接向镜像仓库推送（不依赖 Docker） |
| `proxy.go` / `proxy_windows.go` / `proxy_other.go` | 代理解析；Windows 系统代理读取按平台隔离 |
| `runtime.go` | 容器运行时 → 加载命令与校验命令的映射 |
| `ssh.go` / `ssh_native.go` | 节点传输：系统安全外壳客户端与内置传输层两种实现 |

## 15. 离线镜像交付链路设计（`ldbg bundle`）

### 15.1 要解决的问题

集群是气隙的，Telepresence 的服务端镜像必须由人从外部带进去。原始实现直接调用本机
`docker pull` 与 `docker save`，这带来三个实际问题：

1. **要求本机安装 Docker。** 实际场景是开发工程师在一台全新的 Windows 笔记本上执行这一步。
2. **只会拉本机架构的镜像。** 目标集群可能是 arm64，而构建机是 amd64（或反过来）。
3. **失败时无法定位。** 只有一句"请在有网络且有 Docker 的机器上运行"，对使用者毫无帮助。

### 15.2 三种获取方式与自动选择

```mermaid
graph LR
    start["ldbg bundle"] --> auto{"--engine"}
    auto -->|docker| dk["docker pull + docker save"]
    auto -->|native| nt["内置：直接与镜像仓库通信"]
    auto -->|auto（默认）| chk{"本机有可用 Docker<br/>且该平台镜像已在本地？"}
    chk -->|是| save["docker save（完全不联网，最快）"]
    chk -->|否| nt
    nt --> tar["docker-archive 格式 tar 包"]
    dk --> tar
    save --> tar
```

**为什么 `auto` 优先使用"本地已有镜像"**：这是最省事也最可靠的路径——不联网、不受代理与网络波动影响。
实际场景中，使用者常常已经手工执行过 `docker pull`，此时再联网拉一次纯属浪费且可能失败。

**内置拉取的实现要点**：使用容器镜像库直接与仓库通信，从多架构索引中**精确选择**请求的架构，
并以官方镜像名写出与 `docker save` 同格式的归档。因此无论从哪个源拉取，产出物都能被
`docker load`、`ctr images import`、各类本地集群的加载命令读取。

### 15.3 平台选择：为什么默认不是本机架构

默认值定为 `linux/amd64`，而不是"跟随构建机"。理由是：**这个包是给集群用的，不是给构建它的机器用的。**
跟随本机架构会在 Apple 芯片的 Mac 上静默产出 arm64 包，导入 amd64 集群后表现为容器启动失败，
错误信息（可执行文件格式错误）与真正的原因相距很远，排查成本高。

配套约束：**一个包只装一种架构**。传入逗号分隔的多架构会明确报错并说明分两次执行。
这样所有导入路径的行为都一致可预期，不需要为"多架构归档在某个导入工具下是否可用"逐一验证。
未显式指定输出文件名时，非默认架构的文件名自动带上架构后缀，避免两次打包互相覆盖。

### 15.4 代理解析：为什么必须读 Windows 系统代理

**根本原因**：Go 语言的标准代理解析只读环境变量，**不读 Windows 的系统代理设置**。
而 Clash、v2rayN 这类工具正是配置系统代理。结果是浏览器能上网、本工具却直连被墙，
使用者看到的只是一句连接超时，完全无法把"我开着代理"和"工具没用它"联系起来。

解析优先级与展示规则：

```
--proxy 参数  >  HTTPS_PROXY / HTTP_PROXY 环境变量  >  Windows 系统代理（读注册表）  >  直连
```

每次拉取都打印实际生效的代理及其来源，**密码位置打码**。检测到自动配置脚本（PAC）时不强撑，
明确提示无法解析并要求手工指定。

**凭证必须独立传入**的两个理由（均为实际问题）：

1. Windows 注册表里**只有代理地址，没有账号密码**（凭证在凭据管理器中，程序取不到），
   因此自动发现出来的代理遇到需要认证的代理必然被拒绝。
2. 把凭证拼进地址，在密码含 `@`、`/`、`:` 时会被**静默解析错误**——主机名都会变，且不报错。
   独立参数配合正确的编码函数可以避免这个陷阱。

### 15.5 失败分类：为什么这是一个独立的设计点

原始实现把底层错误原样抛出，使用者无法据此行动。现在对错误串做分类，每类给出唯一的下一步：

| 分类 | 判定依据（要点） | 给出的下一步 |
| --- | --- | --- |
| 域名解析失败 | 出现"lookup"且伴随超时、无此主机等症状 | 检查网络与虚拟专用网；本地已有镜像则免拉取；或换源 |
| 网络不可达 | 连接被拒绝、无路由、连接重置，**以及 Windows 的对应表述** | 设置代理；换可达的镜像源；或在能联网的机器上打包后拷贝 |
| 代理要求认证 | 返回代理认证要求 | 指向**代理凭证**参数（而不是仓库凭证） |
| 仓库拒绝认证 | 未授权、拒绝访问 | 指向仓库凭证参数 |
| 镜像或架构不存在 | 清单未知、索引中无该平台 | 检查版本与架构参数 |
| 传输层安全失败 | 证书相关错误 | 自签名仓库使用放宽参数，或安装证书颁发机构证书 |
| 本机 Docker 不可用 | 无法连接 Docker 守护进程、找不到可执行文件 | 提示默认引擎根本不需要 Docker |

**两个必须固定的顺序，都是实际踩过的坑：**

1. **域名解析必须先于认证判断。** Docker 会把"获取匿名令牌失败"包在"授权失败"里，
   因此一次纯粹的域名解析故障会同时出现"授权失败"与"解析超时"两种字样。
   若先判认证，使用者会被带去排查根本不存在的凭证问题。
2. **代理认证必须先于仓库认证判断。** 代理返回的认证要求同样包含"需要认证"字样，
   若先判仓库认证，会把人指向完全无关的仓库凭证参数。

此外，域名解析类失败时，工具会**用自己的解析器再解析一次**，从而区分两种情况：
本机整体解析不了（网络或虚拟专用网问题），还是只有 Docker 守护进程解析不了（守护进程的代理配置问题）。
这两种情况的修复动作完全不同。

## 16. 集群侧导入设计（`ldbg cluster install`）

### 16.1 要解决的问题

打包只完成了一半，另一半是把归档送进集群节点的容器运行时。原始实现的三个分支都不适配真实场景：
本地开发集群的加载命令只适用于单机集群；仓库路径要求运维机安装 Docker；
containerd 分支实际上只打印一句手工说明就返回错误，等于没有实现。同时缺三样东西：
不知道节点跑的是什么运行时、没有任何传输手段、没有"哪些节点需要这个镜像"的概念。

### 16.2 节点事实来自集群，不来自使用者

节点的容器运行时**已经记录在集群里**（节点状态中的容器运行时版本字段，形如 `docker://29.2.1`）。
因此工具直接读取它，而不是要求使用者查清楚再手工填写。同时读取的还有：
处理器架构（决定该打哪个架构的包）、内部地址（决定往哪里传输）、
是否可调度（决定是否需要导入）、云提供商标识（用于识别本地开发集群）。

### 16.3 运行时到命令的映射

| 运行时 | 导入命令 | 校验命令 |
| --- | --- | --- |
| containerd | `ctr -n k8s.io images import <归档>` | 列出镜像后匹配镜像名 |
| docker | `docker load -i <归档>` | `docker image inspect <镜像>` |
| cri-o | `podman load -i <归档>` | `crictl images -q <镜像>` |

**必须写明的三个细节：**

1. **containerd 必须导入 `k8s.io` 命名空间。** 导入到默认命名空间会返回成功，
   但 kubelet 看不到该镜像，集群表现为拉取镜像失败。这是一个不报错的静默失败，是本设计中最危险的一处。
2. **k3s 与 RKE2 内嵌 containerd，没有独立的 `ctr` 命令。** 因此导入语句在节点上按顺序探测
   `ctr`、`k3s ctr`、`nerdctl`，都不存在时给出明确提示。
3. **识别不出运行时时不再直接拒绝**（v0.3.5 起，见 §16.7）：集群列表里的节点转入
   节点上探测；只有通过 `--nodes` 手工给出且不在集群列表里的地址才要求
   `--runtime` 或 `--import-cmd`——那可能是拼写错误，把镜像装进错误的机器不可接受。

### 16.3.1 kubelet 上报非标准字符串时：强制指定与节点上探测（v0.3.5）

上面的映射表默认 kubelet 上报的是标准形式（`docker://29.2.1` 这样）。真实故障
（2026-08-04）：某定制化平台即服务集群的节点实际运行 docker，但 kubelet 上报的
运行时字符串不是标准形式，解析结果为"未知"，工具在**已经能通过安全外壳协议登录节点**
的情况下直接放弃，且报错只说"未知"，把 kubelet 的原始字符串丢掉了，使用者无从诊断。

修复由三层组成（对应 SPEC 的 FR-E7）：

1. **原始字符串永不丢弃。** 目标结构携带 kubelet 原文，进度行显示
   `unrecognized runtime "某字符串" — probing the node`；预检命令对识别不出的节点
   单独提示原文并给出下一步。
2. **`--runtime docker|containerd|cri-o` 强制指定**（首选出口）。只接受三个已知取值，
   拼写错误立刻报错——这个参数的意义是消除猜测，不能让一个错字又把猜测悄悄带回来。
   强制指定后走正常映射表，逐节点校验保留。
3. **零参数时在节点上探测**（兜底出口）。既然节点可登录，探测实际引擎的成本只是
   一条远程命令。探测脚本仍是**一次连接**（延续 §16.6 的原则），按
   `docker → ctr → k3s ctr → nerdctl → podman → isula` 的顺序找第一个存在的命令行工具，
   用它导入 + 校验（containerd 分支仍用 `k8s.io` 命名空间），状态标记扩展为
   `ldbg-status i=$i v=$v rt=$rt s=$s`（`rt` = 命中的引擎，`s` = skip-present 命中），
   结果在输出中标注为 `docker (probed)`。

**探测顺序为什么 docker 排最前**：走到探测这一步，说明 kubelet 的上报不标准——
这类环境绝大多数是改名的 docker 分支（独立部署的 containerd 与 cri-o 会正常上报标准形式）。
更重要的是反例：docker 自带一个受它管理的 containerd，若先探测 `ctr`，
在"docker 为主、containerd 只是其内部组件"的节点上会把镜像装进 kubelet 永远不看的地方。

### 16.4 两条路径与自动选择

```mermaid
graph TB
    s["ldbg cluster install --import-via auto"] --> r{"给了 --registry？"}
    r -->|是| reg["推送到内部仓库<br/>各节点自行拉取<br/>（内置推送，不需要 Docker）"]
    r -->|否| n{"给了 --ssh-user 或 --nodes？"}
    n -->|是| ssh["逐节点：传输 → 导入 → 校验 → 清理"]
    n -->|否| l{"识别出单节点开发集群？"}
    l -->|是| tool["使用该集群自带的镜像加载命令"]
    l -->|否| stop["停下来说明三条路径各自需要什么<br/>（不猜）"]
```

**为什么自动选择不会自行选择逐节点路径**：登录共享集群的每一个节点是有影响面的操作，
必须由使用者明确表达（给出登录用户或节点列表）。工具宁可停下来问，也不推断这种意图。

### 16.5 覆盖面：为什么默认是全部可调度节点

traffic-manager 只有一个副本，看上去只需要一个节点有镜像。但**注入的 traffic-agent 使用同一个镜像**，
并且它会跟随被拦截的工作负载调度到任意节点。少导一个节点，就意味着某次拦截时 Pod 恰好落在那里，
表现为镜像拉取失败——而集群是气隙的，拉不到就是拉不到，且这个失败发生在调试进行到一半的时候。

因此默认导入**全部可调度节点**（被隔离的节点与带有人为调度限制的节点排除），
允许缩小范围，但未完全覆盖时必须警告，并在结构化输出中以布尔字段体现。

### 16.6 单节点流程与往返次数

每个节点的处理是四个阶段：传输、导入、校验、清理。这四步曾经对应四条独立连接，
现在**导入、校验、清理合并为一条远程脚本**，单节点由四次往返降为两次。

合并带来的风险是丢失阶段级诊断——"一条命令失败了"无法区分是导入失败还是校验失败。
解决办法是脚本回传各阶段的退出码：

```sh
<导入命令>; i=$?; v=0; if [ $i -eq 0 ]; then <校验命令>; v=$?; fi; rm -f "<归档>"; echo "ldbg-status i=$i v=$v"; ...
```

调用方解析这一行，因而**在一次往返内仍能精确区分**导入失败与校验失败。
校验受导入退出码保护，避免"导入失败但节点上恰好有旧镜像"被误判为成功。
...
### 16.8 traffic-manager 的命名空间：一个取值，三处使用（v0.3.7）

命名空间原先是编译期常量 `ambassador`。共享集群常常不允许开发者自建命名空间，
所以它必须可配置：全局参数 `--manager-namespace`，以及环境变量 `LDBG_MANAGER_NAMESPACE`
（优先级为参数 > 环境变量 > 默认值；给 AI 代理和持续集成用环境变量，省去每条命令重复书写）。

**关键约束是"三处必须一致"**：安装时写进 helm 的命名空间、连接时传给
`telepresence connect --manager-namespace` 的值、以及读取 manager 日志时查询的命名空间。
而 `--manager-namespace` **会覆盖客户端配置文件里的设置**——这意味着工具内部只要有一处漏改，
客户端就会去一个没有 manager 的命名空间寻找，且报错与真实原因相去甚远。
因此实现上只有一个解析函数，四个使用点（含版本信息输出）全部调用它，
卸载操作也必须显式携带命名空间。

**引入一个旋钮，就等于引入一种配错的方式**，所以 `doctor` 同时增加了一项检查：
直接查询该命名空间里有没有 traffic-manager 的 Pod，并区分两种情形给建议——
...
Telepresence 基于 Kubernetes 官方客户端库实现，**没有 kubeconfig 就完全无法工作**，
不存在"用节点上的 kubectl 代替"这种方案。因此当笔记本无法直连集群应用程序接口时，
唯一出路是把凭证取回本机。
...
| 路径 | 做法 | 能力范围 |
| --- | --- | --- |
| 日志直连 | 在跳板机上做端口转发，笔记本经隧道指定日志存储地址 | 只能查日志，但最稳，不需要本机 kubectl 与任何凭证 |
| 无凭证的接口代理 | 跳板机运行接口代理，笔记本经隧道使用最小 kubeconfig | 只读接口操作；**完整拦截应在跳板机上执行** |
| **取回真实凭证（推荐）** | 经安全外壳从节点导出自包含 kubeconfig，改写为走本地隧道 | **完整能力**，包括在笔记本上执行完整拦截 |

### 17.3 凭证改写的两个要点

第一，导出必须是**自包含**的：所有证书内联，不引用节点上的文件路径，否则拿回本机就是一堆断链。

第二，改写服务器地址为本地隧道端口的同时，**必须把校验用的服务器名固定为原始地址**。
集群证书不是签给 `127.0.0.1` 的，如果只改地址就会校验失败；而直接关闭校验是不必要的安全降级。
固定校验名之后，传输层安全校验依然完整。

### 17.4 为什么完整拦截可以在笔记本上运行

Telepresence 通过**集群应用程序接口的端口转发**到达 traffic-manager，
而不是要求笔记本能直接访问 Pod 网络。安全外壳的本地端口转发是透明的传输控制协议管道，
因此流升级可以正常通过。这一点已通过实测确认：经隧道的连接检查中，端口转发一项通过。

## 18. 面向 AI 编码代理的设计约束

本工具的一个明确目标是可由 AI 编码代理无人值守驱动。代理与人类使用者的差别不在偏好，
而在**能力边界**：代理没有终端，无法回答任何交互式提示，也无法阅读为人排版的输出。
这条边界产生了四项硬性设计约束。

### 18.1 结构化输出与退出码

所有命令支持统一 JSON 外壳，失败时退出码非零且提示可执行。
两个 `ok` 的语义必须区分清楚：外壳的 `ok` 表示命令本身是否跑通，
`data.ok` 是多项检查的聚合结论。混用会导致代理做出错误判断。

### 18.2 绝不因等待交互而挂起

**这是代理场景下最严重的失败模式。** 操作系统自带的安全外壳客户端只从终端读取密码，
代理环境下既看不到提示也答不了，结果是命令永久挂起。

设计做法：检测到没有终端时，自动为安全外壳调用加上批处理模式与连接超时。
效果是把"无限期挂起"变成"数秒内的明确报错"——实测中一个不可达节点由 2 分 16 秒降至 5.0 秒。
同时提供显式参数，让人类使用者可以恢复交互式行为。

**这里有一个必须写明的实现陷阱**：判断是否有终端必须使用真正的终端检测，
而不是检查"标准输入是否为字符设备"。`/dev/null` 也是字符设备，
用后者判断会把代理的重定向输入误判为交互式环境，从而使上述保护完全失效。
这个错误在实现中真实发生过，症状正是保护未生效、不可达节点仍然挂满 2 分 16 秒。

### 18.3 内置安全外壳传输层

既然操作系统自带的客户端在无终端环境下无法使用密码，工具就自己实现了传输层。
密码**只从环境变量 `LDBG_SSH_PASSWORD` 读取，不提供命令行参数**
（命令行参数会进入进程列表、命令历史与持续集成日志）。

其它设计取舍：
- 每个节点只认证一次，一条连接完成上传与命令执行。
- 文件传输走管道写入而不是自行实现文件复制协议——效果相同，协议面小得多。
- 主机密钥策略：**已记录且不匹配则直接拒绝**（这是真正要防的），未记录则打印指纹后本次接受
  （气隙集群节点通常不在开发者笔记本的已知主机记录中），并提供严格模式参数。
- 默认仍使用系统客户端，因为它承载使用者自己的配置（跳板机、代理命令、密钥代理）；
  只有在设置了密码环境变量或显式指定时才切换到内置实现。
...
| 编号 | 决策点 | 结论 | 理由 |
| --- | --- | --- | --- |
| D9 | 笔记本无 kubeconfig 时的方案 | 从集群节点取回真实凭证并改写为走隧道 | Telepresence 基于官方客户端库，无 kubeconfig 无法工作；无凭证的接口代理只能支撑只读操作 |
| D10 | 改写后的传输层安全 | 改地址 + 固定校验服务器名，**保留完整校验** | 证书不是签给回环地址的；关闭校验是不必要的降级 |
| D11 | 打包是否依赖 Docker | **不依赖**，内置与镜像仓库直接通信的能力；本机有 Docker 且镜像已在本地时才用它 | 真实场景是全新 Windows 笔记本；同时保留"完全不联网"的最快路径 |
| D12 | 容器镜像库版本 | 固定在声明 Go 1.18 的版本 | 更高版本要求 Go 1.23/1.25，会顶掉本仓库的 Go 1.22 基线与持续集成配置 |
| D13 | 打包目标架构默认值 | **固定为 `linux/amd64`**，不跟随本机 | 包是给集群用的；跟随本机会在 Apple 芯片上静默产出错误架构 |
| D14 | 多架构归档 | **一包一架构**，多架构参数明确报错 | 让所有导入路径行为一致可预期；需要两种架构就执行两次 |
| D15 | 镜像源替换 | 支持从任意源拉取，但**产出物始终以官方镜像名写出** | 集群侧安装步骤完全不用改动 |
| D16 | Windows 系统代理 | **主动读取注册表**中的系统代理设置 | Go 标准库只读环境变量；代理模式的虚拟专用网客户端配置的正是系统代理，不读就等于没有代理 |
| D17 | 代理与仓库凭证 | 独立参数，不允许拼进地址；密码一律打码 | Windows 不存储代理凭证；含特殊字符的密码拼进地址会被静默解析错误 |
| D18 | 失败分类顺序 | 域名解析先于认证；代理认证先于仓库认证 | 两者的错误串互相包含，顺序错了会把使用者引向不存在的问题 |
| D19 | 节点运行时来源 | **从集群读取**，不要求使用者指定 | 集群已经记录了答案；手工填写是多余负担与出错点 |
| D20 | 导入覆盖面 | 默认全部可调度节点 | 注入的 traffic-agent 与服务端同镜像，会跟随工作负载调度到任意节点 |
...
```bash
# ―― 调试闭环 ――
ldbg doctor <服务> -n <命名空间>          # 预检：客户端、集群、traffic-manager、ambient、日志栈
ldbg up <服务> -n <命名空间>              # 同步环境变量 + 连接 + ambient 豁免 + 全量拦截
ldbg test <服务> -n <命名空间>            # 从集群内部证明流量已到本机
ldbg status                               # 连接与拦截状态
ldbg down                                 # 退出拦截、卸载 agent、还原 ambient、清理

# ―― 日志（全部支持 --json）――
ldbg logs query <服务> --since 4h -q Exception    # 集群日志库（含已删除 Pod 的历史）
ldbg logs local <服务> --level error              # 拦截期间的本机日志
ldbg logs tail / stats / fields / values          # 实时流 / 聚合 / 字段自省

# ―― 远程接入（笔记本无法直连集群接口时）――
ldbg cluster fetch-kubeconfig --ssh <用户>@<节点>  # 取回真实凭证并改写为走本地隧道（推荐）
ldbg cluster probe                                 # 逐项检查桥接能力
ldbg cluster tunnel --bastion <用户>@<主机>        # 打印隧道接入命令
ldbg cluster kubeconfig --api <本地代理地址>       # 生成最小 kubeconfig

# ―― 离线镜像交付（打包侧，可在无 Docker 的机器上执行）――
ldbg bundle                                        # 默认 linux/amd64
ldbg bundle --platform linux/arm64                 # arm64 集群
ldbg bundle --from <镜像源> --creds <用户:密码>     # 换源（产出物仍为官方镜像名）
ldbg bundle --proxy <地址> --proxy-creds <用户:密码>  # 需要认证的代理

# ―― 集群侧导入与安装 ――
ldbg cluster preflight                             # 逐节点列出运行时、架构、地址与建议
ldbg cluster install --bundle <包> --registry <内部仓库>        # 推送到仓库，各节点自行拉取
ldbg cluster install --bundle <包> --ssh-user <用户> --dry-run  # 逐节点导入，先审阅命令
LDBG_SSH_PASSWORD=<密码> ldbg cluster install --bundle <包> --ssh-user <用户>   # 无终端环境
```
